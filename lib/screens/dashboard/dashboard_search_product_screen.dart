...
class _DashboardSearchProductScreenState
  ...
  void initState() {
    super.initState();
    // make sure bottom nav shows "Search" as active
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Column(
      children: [
        ...
        Padding(
          padding: EdgeInsets.symmetric(
            horizontal: 24 * scale.clamp(0.85, 1.2),
            vertical: 10 * scale.clamp(0.85, 1.2),
          ),
          child: Row(
        ...
        Expanded(
          child: ListView.separated(
            padding: EdgeInsets.symmetric(
              horizontal: 20 * scale.clamp(0.85, 1.2),
              vertical: 8 * scale.clamp(0.85, 1.2),
            ),
            physics: const BouncingScrollPhysics(),
            itemCount: state.results.length,
            separatorBuilder: (_, __) =>
                SizedBox(height: 16 * scale.clamp(0.85, 1.2)),
...
class _ProductCardState extends ConsumerState<_ProductCard> {
  ...
  void _toggleMenu() {
    ...
    _overlayEntry = OverlayEntry(
      builder: (_) => _CardMenu(
        ...
        onSeeDetails: () { _closeMenu(); _openDetailSheet(); },
        onAddToList: () { _closeMenu(); _showAddToListModal(); },
        onAddToRecipe: () { _closeMenu(); _showAddToRecipeModal(); },
        onFavorite: () {
          _closeMenu();
          ref.read(dashboardSearchProvider.notifier).toggleFavorite(widget.result.product.id);
        },
      ),
    );
  ...
  void _closeMenu() { _overlayEntry?.remove(); _overlayEntry = null; }
  ...
  void _showAddToListModal() {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _AddToListModal(lists: widget.shoppingLists, scale: widget.scale),
    );
  }
  ...
  void _showAddToRecipeModal() {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _AddToRecipeModal(recipes: widget.recipes, scale: widget.scale),
    );
  }
  ...
  @override
  Widget build(BuildContext context) {
    final eco  = widget.result.badges.ecoLabel;    // e.g. 'Unsustainable'
    final safe = widget.result.badges.safetyLabel;  // e.g. 'Avoid'
    ...
    return GestureDetector(
      ...
      child: Stack(
        children: [
              ...
              children: [
                ...
                Expanded(
                  child: Container(
                    ...
                    padding: EdgeInsets.only(top: 10 * s, bottom: 25 * s),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.eco_outlined, color: Colors.white, size: 24 * s),
                        SizedBox(width: 8 * s),
                        Flexible(
                          child: Text(
                            eco,
                ...
                Expanded(
                  child: Container(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.flag_outlined, color: Colors.white, size: 24 * s),
                        SizedBox(width: 8 * s),
                        Flexible(
                          child: Text(
                            safe,
          ...
          // ── WHITE CARD LAYER (Foreground) ──────────────────────
          Container(
            margin: EdgeInsets.only(top: 45 * s),
            decoration: BoxDecoration(
              color: AppColors.pureWhite,
            ...
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  // Product thumbnail with cream background
                  Container(
                    width: 90 * s,
                    height: 90 * s,
                    decoration: BoxDecoration(
                      color: const Color(0xFFFFF9C4).withValues(alpha: 0.3), // Light cream
...
// ─────────────────────────────────────────────────────────────────
// Pill Badge (inside detail sheet)
// ─────────────────────────────────────────────────────────────────
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            color: AppColors.pureWhite,
            border: Border.all(
              color: AppColors.clearGrey,   // light grey border
              width: 1.5,
            ),
          ),
          child: Center(
            child: Icon(
              Icons.stop_outlined,           // ← octagon shape matching Figma
              size: 26 * scale,
              color: AppColors.neutralGrey,  // light grey icon
            ),
          ),
        ),
...
// ─────────────────────────────────────────────────────────────────
// Chip Section — Labels and Certifications / Possible allergens
// ─────────────────────────────────────────────────────────────────
    ...
    return Column(
      ...
      children: [
        ...
        SingleChildScrollView(
          ...
          child: Row(
            // Figma shows exactly 4 chips in a single row
            // last one is slightly clipped = use a scrollable row
            children: [
...
// ─────────────────────────────────────────────────────────────────
// Outline Action Button (Add To List / Add To Recipe)
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// Shared Base Modal (rounded card with X close + save button)
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// Checkbox Row (shared by list & recipe modals)
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// Filter Preferences Sheet (2.0.7)
// ─────────────────────────────────────────────────────────────────
